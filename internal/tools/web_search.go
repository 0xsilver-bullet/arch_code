...
func ExecuteWebSearch(cwd string, params WebSearchParams) string {
	if params.Query == "" {
	...
	if err != nil {
		return fmt.Sprintf("error: failed to execute search: %v", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusAccepted {
		return fmt.Sprintf("error: search failed with status code: %d", resp.StatusCode)
	}
	...
	if err != nil {
		return fmt.Sprintf("error: failed to parse results: %v", err)
	}
